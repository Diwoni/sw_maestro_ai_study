...
export function ReportView({
  ...
  if (!report) {
    return (
      <section id="main-report" className="empty-report overflow-hidden border border-line bg-white/50">
        <div className="grid min-h-64 lg:grid-cols-[0.72fr_1.28fr]">
          <div className="flex flex-col justify-between border-b border-[#cfd8f5] bg-action-soft p-6 lg:border-b-0 lg:border-r sm:p-8">
            <p className="editorial-kicker">Output / Report</p>
            <span className="mt-12 font-mono text-6xl font-medium tracking-[-0.08em] text-[#c5cff0]">
              00
            </span>
          </div>
          <div className="flex flex-col justify-center p-6 sm:p-10 lg:p-12">
            <p className="max-w-xl text-balance text-2xl font-semibold leading-snug tracking-[-0.035em] text-ink sm:text-3xl">
              {emptyMessage}
            </p>
            <div className="mt-8 grid max-w-lg gap-3 sm:grid-cols-3">
              {["핵심 용어", "합의 질문", "착수 체크"].map((label, index) => (
                <div key={label} className="border-t border-[#cfd8f5] pt-3">
  ...
  return (
    <section id="main-report" className="print-report report-paper border border-line bg-[#fffefa]">
      <div className="grid border-b border-line lg:grid-cols-[1fr_auto]">
        <div className="p-6 sm:p-9 lg:p-11">
          <p className="inline-flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.17em] text-brand">
            <FileText className="h-3.5 w-3.5" strokeWidth={1.8} />
            ContextBridge / Analysis report
          </p>
          <h2 className="mt-6 max-w-3xl text-balance text-3xl font-semibold leading-tight tracking-[-0.045em] text-ink sm:text-4xl">
            협업 문맥 오해 가능성 분석
          </h2>
        ...
        <div className="flex flex-col justify-between border-t border-line bg-[#f3f0e9] p-6 lg:min-w-[320px] lg:border-l lg:border-t-0 sm:p-8">
          <div className="print-hidden flex gap-2">
            <button
              type="button"
              onClick={() => window.print()}
              className="inline-flex flex-1 items-center justify-center gap-2 rounded-md border border-action/30 bg-white px-3 py-2.5 text-xs font-semibold text-action transition-all duration-300 hover:border-action hover:bg-action-soft active:scale-[0.98]"
            ...
            <button
              ...
              className="inline-flex flex-1 items-center justify-center gap-2 rounded-md bg-action px-3 py-2.5 text-xs font-semibold text-white shadow-[0_8px_20px_rgba(49,87,213,0.22)] transition-all duration-300 hover:bg-action-hover active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50"
          ...
          <div className="mt-8 grid grid-cols-3 divide-x divide-line border-y border-line py-4 lg:mt-12">
      ...
      <div className="p-6 sm:p-9 lg:p-11">
        <div className="grid gap-10 border-b border-line pb-10 lg:grid-cols-2 lg:gap-16">
          ...
          <ReportSection number="03" title="오해 가능 용어 분석" spacious>
            <div className="overflow-x-auto border border-line">
              <table className="w-full min-w-[900px] border-collapse text-left text-sm">
                <thead className="bg-[#263b78] text-[10px] font-semibold uppercase tracking-[0.12em] text-white/75">
                  <tr>
                ...
                <tbody className="divide-y divide-line">
                  {report.terms.map((term, index) => (
                    <tr
                      ...
                      className="align-top transition-colors hover:bg-[#f8f5ef]"
                    >
                      ...
                      <td className="p-4 text-muted">
                        ...
                        <p className="mt-4 border-l-2 border-warning bg-amber-50/70 py-2 pl-3 pr-2 text-xs font-medium leading-5 text-ink">
                          {term.riskReason}
                        </p>
                      </td>
...
function ReportSection({
  ...
  return (
    <section className={spacious ? "mt-12" : ""}>
    ...
    >
      ...
      <ol className="mt-6 grid gap-0">
        {items.map((item, index) => (
          <li
            key={item}
            className={`grid grid-cols-[2rem_1fr] gap-3 border-t py-4 text-sm font-medium leading-6 last:border-b ${
              accent ? "border-amber-200" : "border-emerald-200"
            }`}
          >
...
function RoleView({ role, value }: { role: string; value: string | null }) {
  return (
    <p className="border-b border-line/70 py-1.5 last:border-b-0">
